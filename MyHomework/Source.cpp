...
double angle;
GLuint Front ,Back ,Left ,Right ,Up ,Down,carpet,door,face,grassImageID,streetImageID;
float XfocusCamera;
float ZfocusCamera;
float xCamera;
float yCamera;
...
void initCameraPosition();
 ...
 POINT mouse;
 int midscreenx = 200, midscreeny = 300;
 void mousefun(){

	 GetCursorPos(&mouse);
	 //cout << "Hello world!" << endl;
	 if (mouseisactive){
		 if (mouse.x - midscreenx>-1){
			 // cout<<"Right"<<endl;
			 //rotated()
			 MyCamera.RotateY(-5);
		 }
			...
			,float angle,float rotateX,float rotateY,float rotateZ)
{
	...
	glRotatef(angle,0,rotateY,0);
	float hWidth=width/2;
	float hHigh=high/2;
	glBindTexture(GL_TEXTURE_2D,streetImageID);
	glBegin(GL_QUADS);
	glTexCoord2d(0,0);
...
int InitGL(GLvoid)										// All Setup For OpenGL Goes Here
{
	...
	wall=LoadTexture("bricks.bmp");
	streetImageID = LoadTexture("street.bmp");
